...
(function interpreter(parentScope, index, stack, constantPool, bytecode, body, params, option = {}, args) {
    ...
    while (index < bytecode.length) {
        ...
        if ([4, 9, 6, 3, 12, 2, 0, 5, 1, 10, 7, 17, 25, 14, 15, 16, 11, 18, 57].indexOf(instruction) === -1) {
        ...
        if (instruction == 9) {
            ...
            let _body = [];
            let ret = (function () {
                return interpreter(localScope, t1, stack, constantPool, bytecode, _body, _params, {
                    t: this,
                    n: t0,
                    f: t0 || t3,
                    r: 1,
            ...
            _body.push(types.ReturnStatement(ret))
            t4 = types.FunctionExpression(null, _params.reverse(), types.BlockStatement(_body));
            if (t2) {
                stack.push(t4);
            } else {
        ...
        if (instruction == 14) {
            t0 = bytecode[index++];
            t1 = interpreter(localScope, index, stack, constantPool, bytecode, body, params, {
        ...
        if (instruction == 2) {
            ...
            if (t2 === 0) {
                ...
                if (t2.type) {
                    callee = types.MemberExpression(t2.value ? types.Identifier(t2.value) : t2, t1, true)
                } else {
                    callee = types.Identifier(t1.value)
                }
            }
        ...
        if (instruction == 7) {
            ...
            let con = [];
            let tal = [];
            t2 = interpreter(localScope, t1, stack, constantPool, bytecode, con, [])
            t3 = interpreter(localScope, index, stack, constantPool, bytecode, tal, [])
            if (con.length) {
                body.push(types.IfStatement(t0, types.BlockStatement(tal), types.BlockStatement(con)))
                if (t2 > 0) {
                    if (option.r) {
                        return stack.pop();
                    }
                ...
                if (!option.forUpDate) {
                    t2 = interpreter(localScope, t1 + 2, stack, constantPool, bytecode, con, [], {forUpDate: 1})
                }
                if (con.length) {
                    body.push(types.ForStatement(null, t0, con[0].expression, types.BlockStatement(tal)))
                }
            }
            index = t1;
        }
        ...
        if (instruction == 10) {
            ...
            for (t1 = t0; t1 >= 0; t1--) {
                ...
                params.push(types.Identifier(t2.value))
            }
        }
        if (instruction == 11) {
            if (option.r) {
        ...
        if (instruction == 19) {
            ...
            t5 = bytecode[index++];
            try {
                t1 = interpreter(localScope, t3, stack, constantPool, bytecode, body, params, {
                    t: localScope[constantPool[0]],
                    forUpDate: option.forUpDate,
                });
                if (t1 > 0) {
                ...
                t1 = interpreter(localScope, t4, stack, constantPool, bytecode, body, params, {
                    t: localScope[constantPool[0]], e: e,
                    forUpDate: option.forUpDate,
                });
                if (t1 > 0) {
                    if (option.r) {
                ...
                t1 = interpreter(localScope, t5, stack, constantPool, bytecode, body, params, {
                    t: localScope[constantPool[0]],
                    forUpDate: option.forUpDate,
                });
                if (t1 > 0) {
...
})(typeof window !== 'undefined' ? window : (window = global, window), 0, [], constantPool, bytecode, body, []);
//  打印反编译结果
